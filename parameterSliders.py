...
for val in NRArr:	
	...
	counter += 1
# #CurrentMap
path=imDir+"map_LCIn"+str(round(LCI.get(),1)).replace("-","n").replace(".","-")+"_LTW"+str(round(LTW.get(),1)).replace(".","-")+"_NR-"+NR.get()+".bmp"
...
while True:
	...
	if LCI.get() == 0.0:
		...
		path=imDir+"map_LCI"+str(round(LCI.get(),1)).replace("-","n").replace(".","-")+"_LTW"+str(round(LTW.get(),1)).replace(".","-")+"_NR-"+NR.get()+".bmp"
	img=ImageTk.PhotoImage(Image.open(path))
