...
def map_maker(
    ...
    for x in range(width):
        for y in range(height):
            ...
            pixels = im.load()
            neighborProportion = check_neighbors(
                pixels, width, height, x, y, nRange)
        ...
        while NRD <= 10:
            for denomSet in range(0, 3):
                if denomSet == 0 and NRD > 1:
                    continue
                filename = "map_LCI"+str(round(LCI, 1)).replace("-", "n").replace(
                    ".", "-")+"_LTW"+str(round(LTW, 1)).replace(".", "-")
                if denomSet == 0 and NRD == 1:  # nRange = 1
                    ...
                    map_maker(filename, width, height,
                              LCI, LTW, int(height/NRD))
                elif denomSet == 2:  # nRange = width/NRD
                    filename += "_NR-W"+str(NRD)
                    map_maker(filename, width, height,
                              LCI, LTW, int(width/NRD))
                counter += 1
            NRD += 1
        NRD = 1
        LTW += 0.1
